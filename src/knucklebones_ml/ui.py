...
class User_Interface:
    ...
    def select_row(
        self, player: "Player", board: "Board", dice: int
    ) -> Literal[0, 1, 2]:
        raise NotImplementedError

    def show_turn_start(
        self, player_name: str, dice: int, player_type: Literal["human", "ai"] | None
    ) -> None:
        raise NotImplementedError
    ...
    def show_game_end(
        self, player_1_name: str, player_2_name: str, score_1: int, score_2: int
    ) -> None:
        raise NotImplementedError
...
class Test_UI(User_Interface):
    def choose_players(self) -> tuple["Player", "Player"]:
        ...
        if input("Is Player 2 a human? (y/n): ").lower() == "y":
            ...
            p2 = PLAYER_LIST[choice(list(PLAYER_LIST.keys()))]["constructor"](
                "Player 2"
            )
        return (p1, p2)
        ...
        while True:
            try:
                row = (
                    int(
                        input(
                            f"{player.name}, select a row (1-3) to place your {dice}: "
                        )
                    )
                    - 1
                )
    ...
    def show_game_end(
        ...
        print(
            f"Game Over! {player_1_name} scored {score_1}, while {player_2_name} scored {score_2}."
        )
