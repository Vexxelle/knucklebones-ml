...
if __name__ == "__main__":
    a = np.arange(18).clip(0, 6).reshape(2, 3, 3)
    die = 6
    ps = ("Gluipi", "Blario")

    br = BasicRenderer(ps)
    br.render({"board": a, "die": die}, "Gluipi", None, False, False)

    i = br.get_human_action("Gluipi", np.array([1, 0, 0]))
    a = logic.apply_action(die, a, 0, i)
    br.render({"board": a, "die": die}, "Blario", i, True, False)
